...
@Directive({
  selector:
    "input[formControlName][InputMask], input[formControl][InputMask], input[ngModel][InputMask]",
  providers: [
    DecimalPipe,
    CURRENCYDIRECTIVE_VALUE_ACCESSOR,
    CURRENCYDIRECTIVE_VALIDAT0R,
  ],
})
  ...
  implements ControlValueAccessor, Validator, OnInit
{
  ...
  private readonly allowedKeysUsedWithControl: string[] = [
    "x",
    "c",
    "v",
    "a",
  ...
  private numberWithUpTo2DecimalsRegex = new RegExp(
    `^([0-9]+)?(\\${this.decimalPointSeparator}[0-9]?[0-9]?)?$`
  );
  ...
  constructor(
    ...
    this.numbersAndDecimalPointSeparatorRegex = new RegExp(
      `[0-9${localeDecimalPointSeparator}]`,
      "g"
    );
  }
  ...
  handleInput(
    ...
    if (
      ...
      this.hasOnlyOneDecimalSeparatorAtTheEnd(rawInputValue)
    ) {
      return;
    }
    if (rawInputValue === "") {
    ...
    if (
      this.hasNoMoreThan2DecimalDigits(rawInputValue) &&
      !this.isNumberTooLarge(rawInputValue)
    ) {
      const newRawInputValue = this.formatOnlyOnBlur
        ? rawInputValue
        : this.formatPriceToUpTo2Decimal(rawInputValue);
      this.previousRawInputValue = newRawInputValue;
      this.onChange(newRawInputValue);
      this.setNativeInputValue(newRawInputValue);

      const diffInSeparators = this.getDiffInSeparators(
        rawInputValue,
        newRawInputValue
      );
      ...
      const newRawInputValue = this.replaceAt(
        this.previousRawInputValue,
        initialCursorPosition.start - 1,
        enteredData
      );
      this.previousRawInputValue = newRawInputValue;
      this.onChange(newRawInputValue);
  ...
  private replaceAt(originalValue: string, index: number, replacement: string) {
    return (
      ...
      replacement +
      originalValue.substr(index + replacement.length)
    );
  }
  ...
  private isNumberKey(event: KeyboardEvent): boolean | string {
    return (
      event.key &&
      event.key.length === 1 &&
      !!String(event.key).match(this.numbersAndDecimalPointSeparatorRegex)
    );
  }
  ...
  private isSpecialAllowedKey(event: KeyboardEvent): boolean {
    return (
      ...
      this.isAllowedKeyWithControl(event)
    );
  }
  ...
  private isAllowedKeyWithControl(event: KeyboardEvent): boolean {
    if (event.ctrlKey || event.metaKey) {
      return (
        ...
        this.allowedKeysUsedWithControl
  ...
  private hasNoMoreThan2DecimalDigits(value: string): boolean {
    const valueWithoutSeparators =
      this.getValueWithoutThousandsSeparator(value);
  ...
  private formatPriceTo2Decimal(value: number | string): string | null {
    const numberToTransform =
      (typeof value === "string"
        ? this.parseLocaleStringToNumber(value)
        : value) || 0;
  ...
  private formatPriceToUpTo2Decimal(value: string): string {
    ...
    const numberOfZerosAfterDecimalPoint =
      valueWithoutSeparators.match(numberOfZerosAfterDecimalPointRegex)?.[1]
        .length || 0;
    const numberOfZerosAfterFirstDecimalDigit =
      valueWithoutSeparators.match(
        numberOfZerosAfterFirstDecimalDigitRegex
      )?.[1].length || 0;

    const formatted = this.decimalPipe.transform(
      this.parseLocaleStringToNumber(valueWithoutSeparators),
      "1.0-2"
    );
    if (numberOfZerosAfterDecimalPoint > 0) {
      return (
        ...
        "00".slice(0, numberOfZerosAfterDecimalPoint)
      );
    }
  ...
  private getDiffInSeparators(oldValue: string, newValue: string): number {
    const initialNumberOfSeparators =
      this.getNumberOfThousandsSeparators(oldValue);
    const currentNumberOfSeparators =
      this.getNumberOfThousandsSeparators(newValue);
  ...
  private hasOnlyOneDecimalSeparatorAtTheEnd(value: string): boolean {
    return (
      value.split(this.decimalPointSeparator).length === 2 &&
      value.slice(-1) === this.decimalPointSeparator
    );
  }

  private parseLocaleStringToNumber(value: string): number {
    const cleanPattern = new RegExp(
      `[^-+0-9${this.decimalPointSeparator}]`,
      "g"
    );
  ...
  private setCursorPosition(
    ...
    inputEl.setSelectionRange(
      position.start >= 0 ? position.start : 0,
      position.end >= 0 ? position.end : 0
    );
  }

  private moveCursor(
    inputEl: HTMLInputElement,
    initialPosition: ICursorPosition,
    diff: number = 0
  ): void {
  ...
  registerOnChange(fn: (value: number) => void): void {
    this.onChange = (value: string) => {
      const valueToEmit =
        value === "" ? null : this.parseLocaleStringToNumber(value);
      fn(valueToEmit!);
    };
  }
