...
export class CancellationReasonsService {
  ...
  > {
    return this.http.post<IBaseResponse<ICancellationReasons[]>>(
      this.env +
  ...
  getEditCancellationReasons(
    ...
    return this.http.get<IBaseResponse<ICancellationReasonsData>>(
      this.env +
        ApiRoutes.masterTables.BusinessDevelopment.CancellationRejectionReasons
          .CancellationReason.edit,
      { params: { id } }
    );
  }

  DeleteCancellationReasons(id: string): Observable<IBaseResponse<number>> {
    return this.http.post<IBaseResponse<number>>(
      this.env +
        ApiRoutes.masterTables.BusinessDevelopment.CancellationRejectionReasons
