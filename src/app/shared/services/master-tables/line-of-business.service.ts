...
export class LineOfBusinessService {
	...
	getEditLineOfBusinessData(id: string): Observable<HttpResponse<IBaseResponse<ILineOfBusinessData>>> {
		return this.http.get<IBaseResponse<ILineOfBusinessData>>(this.env + ApiRoutes.masterTables.lineOfBusiness.edit, {
			params: { id },
			observe: "response",
		});
	}

	DeleteLineOfBusiness(id: string, LineOfBusiness: string): Observable<HttpResponse<IBaseResponse<number>>> {
		return this.http.post<IBaseResponse<number>>(
			this.env + ApiRoutes.masterTables.lineOfBusiness.delete,
			{},
			{ params: { id, LineOfBusiness }, observe: "response" }
		);
	}
}
