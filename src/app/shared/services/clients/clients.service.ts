...
import { IClientReportReq } from "../../app/models/Clients/iclient-report";
...
export class ClientsService {
	...
	getAllClients(clientFilters: IClientFilters): Observable<HttpResponse<IBaseResponse<IClient[]>>> {
		return this.http.post<IBaseResponse<IClient[]>>(this.env + ApiRoutes.Clients.search, clientFilters, {
			observe: "response",
		});
	}
	getClintDetails(id: string): Observable<HttpResponse<IBaseResponse<IClientPreview>>> {
	...
	viewReport(body: IClientReportReq): Observable<HttpResponse<IBaseResponse<any>>> {
		return this.http.post<IBaseResponse<any>>(this.env + ApiRoutes.Clients.report, body, { observe: "response" });
	}
}
