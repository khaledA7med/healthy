...
import { NgbCalendar, NgbCalendarIslamicUmalqura, NgbDateStruct, NgbInputDatepickerConfig } from "@ng-bootstrap/ng-bootstrap";
...
export class GregorianPickerComponent implements OnChanges {
	...
	constructor(config: NgbInputDatepickerConfig, public changeDate: NgbCalendarIslamicUmalqura, private calendar: NgbCalendar) {
	...
	onDateSelect(e: any) {
		let hijri = this.changeDate.fromGregorian(new Date(`${e.year}-${e.month}-${e.day}`));
		let obj = {
			gon: e,
			hijri,
		};
