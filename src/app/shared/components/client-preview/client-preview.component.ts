...
export class ClientPreviewComponent implements OnInit, OnDestroy {
	...
	getClintDetails(id: string): void {
		...
		let sub = this.clientService.getClintDetails(id).subscribe((res: HttpResponse<IBaseResponse<IClientPreview>>) => {
			if (res.status) {
				this.uiState.clientDetails = res.body?.data!;
				AppUtils.nullValues(this.uiState.clientDetails);
				this.uiState.clientDetails.clientsBankAccounts?.map((b) => AppUtils.nullValues(b));
				this.uiState.clientDetails.clientContacts?.map((c) => AppUtils.nullValues(c));
	...
	initAddClientToGroupForm(): void {
		this.addClientToGroupForm = new FormGroup({
			clientId: new FormControl(this.uiState.clientDetails.sNo, Validators.required),
	...
	addClientToGroupReq(): void {
		let sub = this.clientsGroupService.addClient(this.form["clientId"].value, this.form["groupName"].value).subscribe({
			// no data here
			next: (res: HttpResponse<IBaseResponse<null>>) => {
				if (res.body?.status) {
					this.message.toast(res.body?.message!, "success");
