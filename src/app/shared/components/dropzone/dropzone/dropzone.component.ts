...
export class DropzoneComponent implements OnDestroy {
	...
	constructor(private message: MessagesService, public util: AppUtils, private masterMethod: MasterMethodsService) {}
	...
	removeImage(item: any, isServer?: boolean) {
		this.message.confirm("Sure!", "You Want To Delete ?!", "danger", "question").then((res: any) => {
			if (res.isConfirmed) {
				this.documentsToDisplay = this.documentsToDisplay.filter((doc) => doc.name !== item.name);
				this.documentsToUpload = this.documentsToUpload.filter((doc) => doc.name !== item.name);

				if (isServer) {
					let sub = this.masterMethod.deleteFile(item.data).subscribe((res: HttpResponse<IBaseResponse<boolean>>) => {
						if (res.body?.status) {
							this.message.toast("Deleted!", "info");
							this.emitingFiles();
							this.UploadedFiles = this.UploadedFiles.filter((el) => el.name !== item.name);
	...
	fileIcon(type: string): { cls: string; ico: string } {
		...
		switch (type) {
			case "msword" && "doc" && "docx" && "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
				cls = "bg-soft-secondary text-secondary";
				ico = "ri-file-word-2-fill";
				break;
			case "vnd.ms-powerpoint" && "ppt" && "pptx" && "application/vnd.openxmlformats-officedocument.presentationml.presentation":
			...
			case "zip":
				...
				break;
			case "vnd.ms-excel" && "xls" && "xlsx" && "csv" && "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":
