...
import {
  SalesLeadStatus,
  SalesLeadType,
} from "src/app/shared/app/models/BusinessDevelopment/business-development-util";
...
import { TaskModules } from "src/app/shared/app/models/Activities/task-modules";
...
export class BusinessFormsComponent implements OnInit, OnDestroy {
  ...
  initForm() {
    this.formGroup = new FormGroup<ISalesLeadForm>({
      ...
      currentBroker: new FormControl(
        { value: null, disabled: true },
        Validators.required
      ),
      currentInsurer: new FormControl({ value: null, disabled: true }),
      existingPolDetails: new FormControl(
        { value: null, disabled: true },
        Validators.required
      ),
      //quoting
  ...
  checkCompanyExist(requirementList: any, value?: string): IRequirement {
    return requirementList.find(
      (el: IRequirement) => el.insuranceCopmany == value
    );
  }

  getQuotingReq(data: IRequirementRequest) {
    let check = this.checkCompanyExist(
      this.f?.quotingRequirementsList?.value,
      data.companyName!
    );
    if (!check) {
      let sub = this.businessDevService
        .quotRequirements(data)
        .subscribe((res: HttpResponse<IBaseResponse<IRequirement[]>>) => {
          if (res.body?.status)
  ...
  getPolicyReq(data: IRequirementRequest) {
    let check = this.checkCompanyExist(
      this.f?.policyRequiermentsList?.value,
      data.companyName!
    );
    if (!check) {
      let sub = this.businessDevService
        .policyRequirements(data)
        .subscribe((res: HttpResponse<IBaseResponse<IRequirement[]>>) => {
          if (res.body?.status)
  ...
  getSalesLead(id: string) {
    ...
    let sub = this.businessDevService
      .getSalesLeadById(id)
      .subscribe((res: HttpResponse<IBaseResponse<ISalesLeadDetails>>) => {
        if (res.body?.status) {
          this.eventService.broadcast(reserved.isLoading, false);
          this.patchValuesWhenEdit(res.body?.data!);
          this.businessTasks = res.body.data?.salesLeadTasks!;
        } else {
          this.eventService.broadcast(reserved.isLoading, false);
  ...
  taskControl(task?: ITasks) {
    ...
    this.modalRef.componentInstance.task = {
      module: TaskModules.SalesLead,
      clientID: this.f.clientID?.value,
      clientName: this.f.name?.value,
      moduleSNo: +this.uiState.editId,
      isModule: true,
      ...task,
    } as ITasks;

    let sub = this.modalRef.componentInstance.taskResult.subscribe(
      (res: ITasks) => {
        if (res.sNo) {
          let index = this.businessTasks.findIndex((x) => x.sNo === res.sNo);
          this.businessTasks[index] = res;
          this.businessTasks[index].leadProducer = this.f.producer?.value!;
        } else this.businessTasks.push(res);
      }
    );
    this.subscribes.push(sub);
  }
  ...
  submitForm(form: FormGroup<ISalesLeadForm>) {
    ...
    formData.append(
      "EstimatedPremium",
      salesLeadForm?.estimatedPremium !== null
        ? salesLeadForm?.estimatedPremium?.toString()!
        : "0"
    );
    formData.append(
      "ChDeadlinebool",
      salesLeadForm?.chDeadlinebool?.toString()!
    ); // boolean //check
    formData.append(
      "DeadLine",
      this.utils.dateFormater(salesLeadForm?.deadLine!)
    );
    ...
    formData.append(
      "ExistingPolExpDate",
      this.utils.dateFormater(salesLeadForm?.existingPolExpDate)
    );
    formData.append(
      "CurrentPolicyNo",
      salesLeadForm?.currentPolicyNo! ? salesLeadForm?.currentPolicyNo! : ""
    );
    formData.append(
      "CurrentBroker",
      salesLeadForm?.currentBroker! ? salesLeadForm?.currentBroker! : ""
    );
    formData.append(
      "CurrentInsurer",
      salesLeadForm?.currentInsurer! ? salesLeadForm?.currentInsurer! : ""
    );
    formData.append(
      "ExistingPolDetails",
      salesLeadForm?.existingPolDetails!
        ? salesLeadForm?.existingPolDetails!
        : ""
    );
    ...
    salesLeadForm.salesLeadCompetitorsList?.forEach((el, i) => {
      ...
      formData.append(
        ...
        el.competitorNotes ? el.competitorNotes : ""
      );
    });
    ...
    if (salesLeadForm.sendToUW)
      formData.append("Status", SalesLeadStatus.PendingwithUnderwriting);
