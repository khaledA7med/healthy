...
export class CustomerServiceFormsComponent implements OnInit {
  ...
  setRequestDataToForm(data: ICustomerService) {
    ...
    this.f.existingPolExpDate?.patchValue(
      this.util.dateStructFormat(data.existingPolExpDate!) as any
    );
    ...
    this.f.dateOfDeadline?.patchValue(
      this.util.dateStructFormat(data.dateOfDeadline!) as any
    );
  ...
  fillCSFormData(e: CSPolicyData) {
    ...
    this.formGroup.patchValue({
      ...
      existingPolExpDate: this.util.dateStructFormat(
        this.uiState.requestDetailsFromClient.periodTo!
      ) as any,
  ...
  getSimilarAndRequir(e: any) {
    this.getEndorsTypeByPolicy(
      e.name,
      this.uiState.requestDetailsFromClient.policyNo!
    );
  ...
  getEndorsTypeByPolicy(endorsType: string, policyNo: string) {
    let sub = this.customerService
      .getEndorsTypeByPolicy(endorsType, policyNo)
      .subscribe((res: HttpResponse<IBaseResponse<EndorsTypeByPolicy[]>>) => {
  ...
  requirmentsChecker() {
    let checkAll = this.requirmentsControlArray.controls.every(
      (c) => c.get("itemCheck")?.value === true
    );
  ...
  submitForm(form: FormGroup<ICSForm>) {
    ...
    if (this.uiState.editMode)
      formData.append("RequestNo", this.uiState.editRequestData.requestNo!);
    ...
    formData.append(
      "ClassOfBusiness",
      val.classOfBusiness ? val.classOfBusiness : ""
    );
    formData.append(
      "LineOfBusiness",
      val.lineOfBusiness ? val.lineOfBusiness : ""
    );
    formData.append(
      "ExistingPolExpDate",
      val.existingPolExpDate
        ? this.util.dateFormater(val.existingPolExpDate)
        : ""
    );
    formData.append(
      "RequestDetails",
      val.requestDetails ? val.requestDetails : ""
    );
    formData.append(
      "DateOfDeadline",
      val.dateOfDeadline ? this.util.dateFormater(val.dateOfDeadline!) : ""
    );
    formData.append(
      "NetPremium",
      val.netPremium ? val.netPremium?.toString()! : "0"
    );
    formData.append("VatPerc", val.vatPerc ? val.vatPerc?.toString()! : "0");
    formData.append("VatValue", val.vatValue ? val.vatValue?.toString()! : "0");
    formData.append(
      "PolicyFees",
      val.policyFees ? val.policyFees?.toString()! : "0"
    );
    formData.append(
      "TotalPremium",
      val.totalPremium ? val.totalPremium?.toString()! : "0"
    );
    formData.append("Branch", val.branch ? val.branch! : "");
    val.requiermentsList!.forEach((e, i) => {
      formData.append(
        `RequiermentsList[${i}].checked`,
        e.itemCheck === true ? String(e.itemCheck!) : "false"
      );
    ...
    formData.append(
      "sNo",
      this.uiState.editMode ? this.uiState.editRequestData.sno!.toString() : "0"
    );
