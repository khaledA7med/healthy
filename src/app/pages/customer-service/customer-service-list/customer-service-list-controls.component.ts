    ...
    <div class="col d-flex align-items-center justify-content-center">
      <div ngbDropdown class="d-inline-block">
        ...
        >
          ...
          <button ngbDropdownItem (click)="FollowUp()" class="btn btn-sm">
          ...
          <button
            ...
            *ngIf="
              !(comp.permissions$ | async)?.includes(
                comp.uiState.privileges.ChCustomerServiceReadOnly
              )
            "
          >
          ...
          <button
            ...
            *ngIf="
            ...
            "
          >
            <i class="ri-file-paper-line align-bottom me-2 text-muted"></i> Make
            Invoice
          </button>
          <li
            *ngIf="
          ...
          >
            ...
            <ul class="dropdown-menu dropdown-submenu">
              <li *ngIf="params.data?.status !== status.Pending">
                <a class="btn btn-sm dropdown-item"
                  >{{ status.Pending }} &nbsp; &nbsp; &raquo;</a
                >
                <!-- <button (click)="changeCsStatus(status.Pending)" class="btn btn-sm dropdown-item"></button> -->
                <ul class="dropdown-menu dropdown-submenu">
                  <li
                    *ngFor="
                      let el of (comp.lookupData | async)?.PendingReason
                        ?.content!
                    "
                  >
                    <button
                      (click)="changeCsStatus(status.Pending, el.name)"
              ...
              <li *ngIf="params.data?.status !== status.Close">
                <button
                  (click)="changeCsStatus(status.Close)"
                  class="btn btn-sm dropdown-item"
                >
                  {{ status.Close }}
                </button>
              ...
              <li *ngIf="params.data?.status !== status.Cancel">
                <button
                  (click)="rejectRequets(status.Cancel)"
                  class="btn btn-sm dropdown-item"
                >
                  {{ status.Cancel }}
                </button>
...
export class CustomerServiceListControlsComponent {
  ...
  Edit() {
    this._Router.navigate([
      AppRoutes.CustomerService.edit,
  ...
  makeInvoice() {
    if (this.params.data.status === this.status.Close) {
      ...
      this.message.popup(
        "Warning",
        "Request must be closed before you can make an invoice",
        "warning"
      );
    }
  }
