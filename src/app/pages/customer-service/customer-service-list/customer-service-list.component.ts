...
import { ICustomerServiceFollowUp, ICustomerServiceFollowUpForm } from "src/app/shared/app/models/CustomerService/icustomer-service-followup";
...
export class CustomerServiceListComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		let sub = this.permissions$.subscribe((res: string[]) => {
			if (!res.includes(this.uiState.privileges.ChAccessAllUsersCustomer)) this.f.createdBy!.patchValue(this.auth.getUser().name!);
			if (!res.includes(this.uiState.privileges.ChAccessAllBrancheCustomer)) this.f.branch!.patchValue(this.auth.getUser().Branch!);
		});
