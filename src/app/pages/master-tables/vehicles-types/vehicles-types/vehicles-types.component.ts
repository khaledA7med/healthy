...
export class VehiclesTypesComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.VehiclesTypesService.getVehiclesTypes().subscribe((res: HttpResponse<IBaseResponse<IVehiclesTypes[]>>) => {
				if (res.body?.status) {
	...
	openVehiclesTypeDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.VehiclesTypesService.getEditVehiclesTypes(id).subscribe((res: HttpResponse<IBaseResponse<IVehiclesTypesData>>) => {
				if (res.body?.status) {
	...
	submitVehiclesTypesData(form: FormGroup) {
		...
		const data: IVehiclesTypesData = {
			sNo: this.uiState.editVehiclesTypesMode ? this.uiState.editVehiclesTypesData.sNo : 0,
			vehicleType: formData.vehicleType,
			abbreviation: formData.abbreviation,
		};
		if (!this.validationChecker()) return;
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.VehiclesTypesService.saveVehiclesTypes(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteVehiclesTypes(id: string) {
		let sub = this.VehiclesTypesService.DeleteVehiclesTypes(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
