...
export class ClaimsRejectionReasonsComponent implements OnInit, OnDestroy {
	...
	DeleteClaimsRejectionReasons(sno: number) {
		let sub = this.ClaimsRejectionReasonsService.DeleteClaimsRejectionReasons(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
	...
	submitClaimsRejectionReasonsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ClaimsRejectionReasonsService.saveClaimsRejectionReasons(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
