...
import { ICarsMake, ICarsMakeData } from "src/app/shared/app/models/MasterTables/claims/i-cars-make";
...
export class CarsMakeComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.CarsMakeService.getCarsMake().subscribe((res: HttpResponse<IBaseResponse<ICarsMake[]>>) => {
				if (res.body?.status) {
	...
	openCarsMakeDialoge(sno?: number) {
		...
		if (sno) {
			...
			let sub = this.CarsMakeService.getEditCarsMake(sno).subscribe((res: HttpResponse<IBaseResponse<ICarsMakeData>>) => {
				if (res.body?.status) {
	...
	submitCarsMakeData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.CarsMakeService.saveCarsMake(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteCarsMake(sno: number) {
		let sub = this.CarsMakeService.DeleteCarsMake(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
