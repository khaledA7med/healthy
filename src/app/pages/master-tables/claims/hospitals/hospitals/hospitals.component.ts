...
export class HospitalsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.HospitalsService.getHospitals().subscribe((res: HttpResponse<IBaseResponse<IHospitals[]>>) => {
				if (res.body?.status) {
	...
	openHospitalsDialoge(sno?: number) {
		...
		if (sno) {
			...
			let sub = this.HospitalsService.getEditHospitalsData(sno).subscribe((res: HttpResponse<IBaseResponse<IHospitalsData>>) => {
				if (res.body?.status) {
	...
	submitHospitalsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.HospitalsService.saveHospitals(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteHospitals(sno: number) {
		let sub = this.HospitalsService.DeleteHospitals(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
