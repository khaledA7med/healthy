...
export class TpaListComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.TpaListService.getTpaList().subscribe((res: HttpResponse<IBaseResponse<ITpaList[]>>) => {
				if (res.body?.status) {
	...
	submitTpaListData(form: FormGroup) {
		...
		let sub = this.TpaListService.saveTpaList(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteTpaList(sno: number) {
		let sub = this.TpaListService.DeleteTpaList(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
