...
export class ClaimsGeneralItemsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.ClaimsGeneralItemsService.getClaimsGeneralItems(data).subscribe((res: HttpResponse<IBaseResponse<IClaimsGeneralItems[]>>) => {
				if (res.body?.status) {
	...
	DeleteClaimsGeneralItems(sno: number) {
		let sub = this.ClaimsGeneralItemsService.DeleteClaimsGeneralItems(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
	...
	submitClaimsGeneralItemsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ClaimsGeneralItemsService.saveClaimsGeneralItems(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
