...
export class ClaimsStatusComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.ClaimsStatusService.getClaimsStatus(this.uiState.status).subscribe((res: HttpResponse<IBaseResponse<IClaimsStatus[]>>) => {
				if (res.body?.status) {
	...
	DeleteClaimsStatus(sno: number) {
		let sub = this.ClaimsStatusService.DeleteClaimsStatus(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
	...
	getClaimsStatusData(sno: number) {
		...
		let sub = this.ClaimsStatusService.getEditClaimsStatusData(sno).subscribe((res: HttpResponse<IBaseResponse<IClaimsStatusData>>) => {
			if (res.body?.status) {
	...
	submitClaimsStatusData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ClaimsStatusService.saveClaimsStatus(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
