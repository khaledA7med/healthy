...
@Component({
  selector: "app-legal-status-forms",
  template: `
...
export class LegalStatusFormsComponent {
  private params!: ICellRendererParams;
  private comp!: LegalStatusComponent;
  ...
  Edit() {
    this.comp.openLegalStatusDialoge(this.params.data.identity);
  }
    ...
    this.message
      ...
      .then((result: SweetAlertResult) => {
        if (result.isConfirmed) {
          this.comp.DeleteLegalStatus(this.params.data.identity);
        } else {
          return;
        }
      });
