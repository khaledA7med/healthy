...
export class LegalStatusComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.LegalStatusService.getLegalStatus().subscribe((res: HttpResponse<IBaseResponse<ILegalStatus[]>>) => {
				if (res.body?.status) {
	...
	openLegalStatusDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.LegalStatusService.getEditLegalStatus(id).subscribe((res: HttpResponse<IBaseResponse<ILegalStatusData>>) => {
				if (res.body?.status) {
	...
	submitLegalStatusData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.LegalStatusService.saveLegalStatus(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteLegalStatus(id: string) {
		let sub = this.LegalStatusService.DeleteLegalStatus(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
