...
import { IVehicleColorFilter, IVehicleColorForm, IVehicleColorReq } from "src/app/shared/app/models/MasterTables/production/i-vehicle-color";
...
export class VehiclesColorsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.productionService.getVehicleColor().subscribe((res: HttpResponse<IBaseResponse<IVehicleColorFilter[]>>) => {
				if (res.body?.status) {
	...
	getEditItemData(id: string) {
		let sub = this.productionService.editVehicleColor(id).subscribe((res: IBaseResponse<IVehicleColorReq>) => {
			if (res?.status) {
	...
	deleteItem(id: string) {
		let sub = this.productionService.deleteVehicleColor(id).subscribe((res: IBaseResponse<any>) => {
			if (res?.status) {
				this.gridApi.setDatasource(this.dataSource);
	...
	onSubmit(formGroup: FormGroup<IVehicleColorForm>) {
		...
		let sub = this.productionService.saveVehicleColor(data).subscribe(
