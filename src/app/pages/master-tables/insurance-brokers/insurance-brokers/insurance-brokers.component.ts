...
import { IInsuranceBrokers, IInsuranceBrokersData } from "src/app/shared/app/models/MasterTables/i-insurance-brokers";
...
export class InsuranceBrokersComponent implements OnInit {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.InsuranceBrokersService.getInsuranceBrokers().subscribe((res: HttpResponse<IBaseResponse<IInsuranceBrokers[]>>) => {
				if (res.body?.status) {
	...
	openInsuranceDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.InsuranceBrokersService.getEditInsuranceData(id).subscribe((res: HttpResponse<IBaseResponse<IInsuranceBrokersData>>) => {
				if (res.body?.status) {
	...
	submitInsuranceData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.InsuranceBrokersService.saveInsuranceBrokers(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteInsurance(id: string) {
		let sub = this.InsuranceBrokersService.DeleteInsurance(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
