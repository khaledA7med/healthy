...
export class NationalitiesComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.NationalitiesService.getNationalities().subscribe((res: HttpResponse<IBaseResponse<INationalties[]>>) => {
				if (res.body?.status) {
	...
	openNationalitiesDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.NationalitiesService.getEditNationalities(id).subscribe((res: HttpResponse<IBaseResponse<INationaltiesData>>) => {
				if (res.body?.status) {
	...
	submitNationalitiesData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.NationalitiesService.saveNationalities(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteNationalities(id: string) {
		let sub = this.NationalitiesService.DeleteNationalities(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
