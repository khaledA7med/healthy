...
export class BusinessActivityComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.BusinessActivityService.getBusinessActivity().subscribe((res: HttpResponse<IBaseResponse<IBusinessActivity[]>>) => {
				if (res.body?.status) {
	...
	openBusinessActivityDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.BusinessActivityService.getEditBusinessActivity(id).subscribe((res: HttpResponse<IBaseResponse<IBusinessActivityData>>) => {
				if (res.body?.status) {
	...
	submitBusinessActivityData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.BusinessActivityService.saveBusinessActivity(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteBusinessActivity(id: string) {
		let sub = this.BusinessActivityService.DeleteBusinessActivity(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
