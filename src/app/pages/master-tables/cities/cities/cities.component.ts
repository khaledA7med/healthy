...
export class CitiesComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.CitiesService.getCities().subscribe((res: HttpResponse<IBaseResponse<ICities[]>>) => {
				if (res.body?.status) {
	...
	openCitiesDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.CitiesService.getEditCities(id).subscribe((res: HttpResponse<IBaseResponse<ICitiesData>>) => {
				if (res.body?.status) {
	...
	submitCitiesData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.CitiesService.saveCities(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteCities(id: string) {
		let sub = this.CitiesService.DeleteCities(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
