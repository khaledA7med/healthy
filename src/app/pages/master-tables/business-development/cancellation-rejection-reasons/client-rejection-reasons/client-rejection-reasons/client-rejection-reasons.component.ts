...
export class ClientRejectionReasonsComponent implements OnInit, OnDestroy {
	...
	submitClientRejectionReasonsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ClientRejectionReasonsService.saveClientRejectionReasons(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteClientRejectionReasons(id: string) {
		let sub = this.ClientRejectionReasonsService.DeleteClientRejectionReasons(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
