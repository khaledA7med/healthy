...
} from "src/app/shared/app/models/MasterTables/business-development/cancellation-rejection-reasons/i-cancellation-reasons";
...
export class CancellationReasonsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.CancellationReasonsService.getCancellationReasons().subscribe((res: HttpResponse<IBaseResponse<ICancellationReasons[]>>) => {
				if (res.body?.status) {
	...
	submitCancellationReasonsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.CancellationReasonsService.saveCancellationReasons(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteCancellationReasons(id: string) {
		let sub = this.CancellationReasonsService.DeleteCancellationReasons(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
