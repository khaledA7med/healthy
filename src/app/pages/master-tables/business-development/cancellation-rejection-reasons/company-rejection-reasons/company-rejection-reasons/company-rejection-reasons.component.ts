...
export class CompanyRejectionReasonsComponent implements OnInit, OnDestroy {
	...
	submitCompanyRejectionReasonsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.CompanyRejectionReasonsService.saveCompanyRejectionReasons(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteCompanyRejectionReasons(id: string) {
		let sub = this.CompanyRejectionReasonsService.DeleteCompanyRejectionReasons(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
