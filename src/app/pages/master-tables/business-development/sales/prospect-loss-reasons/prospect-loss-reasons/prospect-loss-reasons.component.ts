...
export class ProspectLossReasonsComponent implements OnInit, OnDestroy {
	...
	@ViewChild("ProspectLossReasonsContent") ProspectLossReasonsContent!: TemplateRef<any>;
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.ProspectLossReasonsService.getProspectLossReasons().subscribe((res: HttpResponse<IBaseResponse<IProspectLossReasons[]>>) => {
				if (res.body?.status) {
	...
	openProspectLossReasonsDialoge(id?: string) {
		...
		this.ProspectLossReasonsModal = this.modalService.open(this.ProspectLossReasonsContent, {
		...
		if (id) {
			...
			let sub = this.ProspectLossReasonsService.getEditProspectLossReasons(id).subscribe(
	...
	submitProspectLossReasonsData(form: FormGroup) {
		...
		const data: IProspectLossReasonsData = {
			sNo: this.uiState.editProspectLossReasonsMode ? this.uiState.editProspectLossReasonsData.sNo : 0,
			reason: formData.reason,
		};
		if (!this.validationChecker()) return;
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ProspectLossReasonsService.saveProspectLossReasons(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteProspectLossReasons(id: string) {
		let sub = this.ProspectLossReasonsService.DeleteProspectLossReasons(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
