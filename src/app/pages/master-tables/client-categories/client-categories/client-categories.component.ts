...
export class ClientCategoriesComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.ClientCategoriesService.getClientCategories().subscribe((res: HttpResponse<IBaseResponse<IClientCategories[]>>) => {
				if (res.body?.status) {
	...
	openClientCategoriesDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.ClientCategoriesService.getEditClientCategories(id).subscribe((res: HttpResponse<IBaseResponse<IClientCategoriesData>>) => {
				if (res.body?.status) {
	...
	submitClientCategoriesData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ClientCategoriesService.saveClientCategories(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteClientCategories(id: string) {
		let sub = this.ClientCategoriesService.DeleteClientCategories(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
