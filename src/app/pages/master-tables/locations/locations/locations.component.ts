...
export class LocationsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.LocationsService.getLocations().subscribe((res: HttpResponse<IBaseResponse<ILocations[]>>) => {
				if (res.body?.status) {
	...
	openLocationsDialoge(id?: string) {
		...
		if (id) {
			...
			let sub = this.LocationsService.getEditLocations(id).subscribe((res: HttpResponse<IBaseResponse<ILocationsData>>) => {
				if (res.body?.status) {
	...
	submitLocationsData(form: FormGroup) {
		...
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.LocationsService.saveLocations(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteLocations(id: string) {
		let sub = this.LocationsService.DeleteLocations(id).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
