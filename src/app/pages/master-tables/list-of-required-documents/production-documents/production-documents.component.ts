...
export class ProductionDocumentsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			if (this.f.policyIssueType?.value!) {
				let sub = this.listOfDocumentsService.getPoliciesDocuments(this.f.policyIssueType?.value!).subscribe(
					(res: HttpResponse<IBaseResponse<IPoliciesDocumentFilter[]>>) => {
						if (res.body?.status) {
	...
	onSubmit(formGroup: FormGroup<IPoliciesDocumentForm>) {
		...
		let sub = this.listOfDocumentsService.savePoliciesDocuments(data).subscribe((res: IBaseResponse<any>) => {
			if (res.status) {
