...
export class ClaimsDocumentsComponent implements OnInit, OnDestroy {
  ...
  dataSource: IDatasource = {
    getRows: (params: IGetRowsParams) => {
      ...
      let sub = this.listOfDocumentsService
        .getClaimsDocuments({
          insurClass: this.f.classOfInsurance?.value!,
  ...
  ngOnInit(): void {
    ...
    this.lookupData = this.table.getBaseData(
      MODULES.MasterTableListOfRequiredDocuments
    );
  }
  ...
  getEditItemData(id: string) {
    ...
    let sub = this.listOfDocumentsService
      .editClaimsDocuments(id)
      .subscribe((res: IBaseResponse<IClaimsDocumentReq>) => {
        if (res?.status) {
