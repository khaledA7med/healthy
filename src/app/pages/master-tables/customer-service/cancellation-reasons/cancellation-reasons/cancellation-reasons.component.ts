...
export class CancellationReasonsComponent implements OnInit, OnDestroy {
	...
	openCancellationReasonsDialoge(sno?: number) {
		...
		if (sno) {
			...
			let sub = this.CancellationReasonsService.getEditCancellationReasons(sno).subscribe(
	...
	DeleteCancellationReasons(sno: number) {
		let sub = this.CancellationReasonsService.DeleteCancellationReasons(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
