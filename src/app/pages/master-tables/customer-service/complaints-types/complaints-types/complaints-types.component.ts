...
export class ComplaintsTypesComponent implements OnInit, OnDestroy {
	...
	@ViewChild("ComplaintsTypesContent") ComplaintsTypesContent!: TemplateRef<any>;
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.ComplaintTypesService.getComplaintTypes().subscribe((res: HttpResponse<IBaseResponse<IComplaintTypes[]>>) => {
				if (res.body?.status) {
	...
	openComplaintsTypeDialoge(sno?: number) {
		...
		this.ComplaintsTypesModal = this.modalService.open(this.ComplaintsTypesContent, {
		...
		if (sno) {
			...
			let sub = this.ComplaintTypesService.getEditComplaintTypes(sno).subscribe((res: HttpResponse<IBaseResponse<IComplaintTypesData>>) => {
				if (res.body?.status) {
	...
	submitComplaintsTypesData(form: FormGroup) {
		...
		const data: IComplaintTypesData = {
			sno: this.uiState.editComplaintsTypesMode ? this.uiState.editComplaintsTypesData.sno : 0,
			type: formData.type,
		};
		if (!this.validationChecker()) return;
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ComplaintTypesService.saveComplaintTypes(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteComplaintsTypes(sno: number) {
		let sub = this.ComplaintTypesService.DeleteComplaintTypes(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
