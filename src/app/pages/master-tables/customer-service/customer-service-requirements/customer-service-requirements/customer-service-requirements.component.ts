import {
	ICompanyRequirementsFilter,
	ICompanyRequirementsFilterData,
} from "./../../../../../shared/app/models/MasterTables/customer-service/i-company-requirements-filter";
import {
...
export class CustomerServiceRequirementsComponent implements OnInit, OnDestroy {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			}).subscribe((res: HttpResponse<IBaseResponse<ICompanyRequirementsFilter[]>>) => {
				if (res.body?.status) {
	...
	submitCompanyRequirementsData(form: FormGroup) {
		...
		let sub = this.CompanyRequirementsService.saveCompanyRequirements(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteCompanyRequirements(sno: number) {
		let sub = this.CompanyRequirementsService.DeleteCompanyRequirements(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
