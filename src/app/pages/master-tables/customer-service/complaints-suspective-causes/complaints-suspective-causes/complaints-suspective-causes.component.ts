...
export class ComplaintsSuspectiveCausesComponent implements OnInit, OnDestroy {
	...
	@ViewChild("ComplaintsSuspectiveCausesContent") ComplaintsSuspectiveCausesContent!: TemplateRef<any>;
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.ComplaintSuspectiveCausesService.getComplaintSuspectiveCauses().subscribe(
	...
	initComplaintsSuspectiveCausesForm() {
		this.ComplaintsSuspectiveCausesForm = new FormGroup<IComplaintSuspectiveCauses>({
	...
	submitComplaintsSuspectiveCausesData(form: FormGroup) {
		...
		const data: IComplaintSuspectiveCausesData = {
			sno: this.uiState.editComplaintsSuspectiveCausesMode ? this.uiState.editComplaintsSuspectiveCausesData.sno : 0,
			suspectiveCause: formData.suspectiveCause,
		};
		if (!this.validationChecker()) return;
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.ComplaintSuspectiveCausesService.saveComplaintSuspectiveCauses(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	DeleteComplaintsSuspectiveCauses(sno: number) {
		let sub = this.ComplaintSuspectiveCausesService.DeleteComplaintSuspectiveCauses(sno).subscribe((res: HttpResponse<IBaseResponse<any>>) => {
			this.gridApi.setDatasource(this.dataSource);
