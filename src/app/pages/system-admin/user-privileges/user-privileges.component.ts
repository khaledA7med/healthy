...
import {
  ...
  IUserRolesForm,
  IPrivilegeRole,
} from "src/app/shared/app/models/SystemAdmin/system-admin-utils";
...
export class UserPrivilegesComponent implements OnInit {
      ...
      let sub = this.systemAdminService.getAllRoles().subscribe(
        (res: HttpResponse<IBaseResponse<IUserRoles[]>>) => {
          ...
          params.successCallback(
            this.uiState.roles.list,
            this.uiState.roles.list.length
          );
  ...
  onGridReady(param: GridReadyEvent) {
    ...
    this.gridApi.setDatasource(this.dataSource);
    if ((this, this.uiState.roles.list.length > 0))
  ...
  globalArrs!: {
    MasterTable: IPrivilegeRole[];
    BussinessDevelopment: IPrivilegeRole[];
    ClientRegistry: IPrivilegeRole[];
    Production: IPrivilegeRole[];
    CustomerService: IPrivilegeRole[];
    ClaimsManagement: IPrivilegeRole[];
    FinancialManagement: {
      generalRoles: IPrivilegeRole[];
      makeNewRoles: IPrivilegeRole[];
      archivesRoles: IPrivilegeRole[];
      approvalsRoles: IPrivilegeRole[];
      reportsRoles: IPrivilegeRole[];
    };
    HumanResources: {
      generalRoles: IPrivilegeRole[];
      payrollRoles: IPrivilegeRole[];
    };
    FixedAssets: IPrivilegeRole[];
    SysAdmin: IPrivilegeRole[];
  };
  ...
  allItemsChecker(modulName: string, val: boolean) {
    switch (modulName) {
      case "MasterTable":
        this.globalArrs.MasterTable.forEach((item: IPrivilegeRole) => {
          if (item.name != "chMasterTablesReadOnly" && val === true)
            item.control.patchValue(true);
          else item.control.patchValue(false);
        });
        break;
      case "BussinessDevelopment":
        this.globalArrs.BussinessDevelopment.forEach((item: IPrivilegeRole) => {
          if (item.name != "chBusinessDevelopmentReadOnly" && val === true)
            item.control.patchValue(true);
          else item.control.patchValue(false);
        });
        break;
      case "ClientRegistry":
        this.globalArrs.ClientRegistry.forEach((item: IPrivilegeRole) => {
          if (
            item.name != "chClientsRegistryAdministratorReadOnly" &&
            val === true
          )
            item.control.patchValue(true);
          else item.control.patchValue(false);
        ...
        break;
      case "Production":
        this.globalArrs.Production.forEach((item: IPrivilegeRole) => {
          if (item.name != "chProductionReadOnly" && val === true)
            item.control.patchValue(true);
          else item.control.patchValue(false);
        });
        break;
      case "CustomerService":
        this.globalArrs.CustomerService.forEach((item: IPrivilegeRole) => {
          if (item.name != "chCustomerServiceReadOnly" && val === true)
            item.control.patchValue(true);
          else item.control.patchValue(false);
        });
        break;
      case "ClaimsManagement":
        this.globalArrs.ClaimsManagement.forEach((item: IPrivilegeRole) => {
          if (item.name != "chClaimsManagementSystemReadOnly" && val === true)
            item.control.patchValue(true);
          else item.control.patchValue(false);
        });
        break;
      case "FinancialManagement":
        ...
        for (key in this.globalArrs.FinancialManagement) {
          if (val === true) {
            this.globalArrs.FinancialManagement[key].forEach((e) =>
              e.control.patchValue(true)
            );
            this.ff.chFinance?.patchValue(true);
          } else
            this.globalArrs.FinancialManagement[key].forEach((e) =>
              e.control.patchValue(false)
            );
        }
        break;
      case "HumanResources":
        ...
        for (hrKey in this.globalArrs.HumanResources) {
          if (val === true) {
            this.globalArrs.HumanResources[hrKey].forEach((e) =>
              e.control.patchValue(true)
            );
        ...
        break;
      case "FixedAssets":
        this.globalArrs.FixedAssets.forEach((item: IPrivilegeRole) => {
          if (item.name != "chFixedAssetsReadOnly" && val === true)
            item.control.patchValue(true);
          else item.control.patchValue(false);
        });
        break;
      case "SysAdmin":
        this.globalArrs.SysAdmin.forEach((item: IPrivilegeRole) => {
  ...
  subItemsChecker(moduleName: string, subMenu: string, val: boolean) {
    let mainChecker: IPrivilegeRole[] = [];
    if (moduleName === "FinancialManagement") {
      this.globalArrs.FinancialManagement[
        subMenu as keyof typeof this.globalArrs.FinancialManagement
      ].forEach((item, i) => {
      ...
      this.globalArrs.HumanResources[
        subMenu as keyof typeof this.globalArrs.HumanResources
      ].forEach((item, i) => {
        if (val) item.control.patchValue(true);
        else item.control.patchValue(false);
      });
  ...
  checkMasterItem(modulName: string, subMenu?: string, val?: boolean) {
    let checkarr!: IPrivilegeRole[];
    let checker: IPrivilegeRole | undefined;
    switch (modulName) {
      case "MasterTable":
        checkarr = this.globalArrs.MasterTable.filter(
          (e) =>
            !(e.name == "chMasterTabels" || e.name == "chMasterTablesReadOnly")
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        );
        if (checker)
          this.globalArrs.MasterTable.find(
            (e) => e.name === "chMasterTabels"
        ...
        break;
      case "BussinessDevelopment":
        checkarr = this.globalArrs.BussinessDevelopment.filter(
          (e) =>
            !(
              e.name == "chBussDevelopment" ||
              e.name == "chBusinessDevelopmentReadOnly"
            )
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        ...
        if (checker)
          this.globalArrs.BussinessDevelopment.find(
            (e) => e.name === "chBussDevelopment"
          )?.control.patchValue(true);
        else
          this.globalArrs.BussinessDevelopment.find(
            (e) => e.name === "chBussDevelopment"
      ...
      case "ClientRegistry":
        checkarr = this.globalArrs.ClientRegistry.filter(
          (e) =>
            !(
              e.name == "chClientsRegistryAdmin" ||
              e.name == "chClientsRegistryAdministratorReadOnly"
            )
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        ...
        if (checker)
          this.globalArrs.ClientRegistry.find(
            (e) => e.name === "chClientsRegistryAdmin"
          )?.control.patchValue(true);
        else
          this.globalArrs.ClientRegistry.find(
            (e) => e.name === "chClientsRegistryAdmin"
        ...
        break;
      case "Production":
        checkarr = this.globalArrs.Production.filter(
          (e) => !(e.name == "chProduction" || e.name == "chProductionReadOnly")
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        );
        if (checker)
          this.globalArrs.Production.find(
            (e) => e.name === "chProduction"
          )?.control.patchValue(true);
        else
          this.globalArrs.Production.find(
            (e) => e.name === "chProduction"
        ...
        break;
      case "CustomerService":
        checkarr = this.globalArrs.CustomerService.filter(
          (e) =>
            !(
              e.name == "chCustomerService" ||
              e.name == "chCustomerServiceReadOnly"
            )
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        ...
        if (checker)
          this.globalArrs.CustomerService.find(
            (e) => e.name === "chCustomerService"
          )?.control.patchValue(true);
        else
          this.globalArrs.CustomerService.find(
            (e) => e.name === "chCustomerService"
        ...
        break;
      case "ClaimsManagement":
        checkarr = this.globalArrs.ClaimsManagement.filter(
          (e) =>
            !(
              e.name == "chClaims" ||
              e.name == "chClaimsManagementSystemReadOnly"
            )
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        ...
        if (checker)
          this.globalArrs.ClaimsManagement.find(
            (e) => e.name === "chClaims"
          )?.control.patchValue(true);
        else
          this.globalArrs.ClaimsManagement.find(
      ...
      case "FinancialManagement":
        ...
        let mainChecker: IPrivilegeRole[] = [];
        if (subMenu != "generalRoles")
          checkarr = chosenSubMenu.filter(
            (e) => e.name !== chosenSubMenu[0].name
          );
      ...
      case "HumanResources":
        ...
        let mmainChecker: IPrivilegeRole[] = [];
        if (subMenu != "generalRoles")
          checkarr = cchosenSubMenu.filter(
            (e) => e.name !== cchosenSubMenu[0].name
          );
        ...
        break;
      case "FixedAssets":
        checkarr = this.globalArrs.FixedAssets.filter(
          (e) =>
            !(e.name == "chFixedAssets" || e.name == "chFixedAssetsReadOnly")
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        );
        if (checker)
          this.globalArrs.FixedAssets.find(
            (e) => e.name === "chFixedAssets"
          )?.control.patchValue(true);
        else
          this.globalArrs.FixedAssets.find(
            (e) => e.name === "chFixedAssets"
        ...
        break;
      case "SysAdmin":
        checkarr = this.globalArrs.SysAdmin.filter(
          (e) => !(e.name == "chSystemAdmin")
        );
        checker = checkarr.find(
          (item: IPrivilegeRole) => item.control.value === true
        );
        if (checker)
          this.globalArrs.SysAdmin.find(
            (e) => e.name === "chSystemAdmin"
          )?.control.patchValue(true);
        else
          this.globalArrs.SysAdmin.find(
      ...
      (res: HttpResponse<IBaseResponse<IUserRolesPrivileges>>) => {
        if (res.body?.status) {
          ...
          for (key in this.uiState.privilagesData) {
            if (key === "sNo")
              this.uiState.convertedPrivilagesData[key] =
                this.uiState.privilagesData[key];
            else if (key === "activePrivileges")
              this.uiState.convertedPrivilagesData[key] =
                this.uiState.privilagesData[key];
            else {
              if (this.uiState.privilagesData[key] === 1)
                this.uiState.convertedPrivilagesData[key] = true;
