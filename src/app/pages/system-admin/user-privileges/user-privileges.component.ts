...
import { IUserRoles, IUserRolesForm, IPrivilegeRole } from "src/app/shared/app/models/SystemAdmin/system-admin-utils";
...
export class UserPrivilegesComponent implements OnInit {
	...
	dataSource: IDatasource = {
		getRows: (params: IGetRowsParams) => {
			...
			let sub = this.systemAdminService.getAllRoles().subscribe((res: HttpResponse<IBaseResponse<IUserRoles[]>>) => {
				if (res.body?.status) {
	...
	submitNewRole(form: FormGroup) {
		...
		let sub = this.systemAdminService.addRole(data).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
	...
	deleteRole(sno: number) {
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.systemAdminService.deleteRole(sno).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
				this.message.toast(res.body!.message!, "success");
				this.gridApi.setDatasource(this.dataSource);
				this.eventService.broadcast(reserved.isLoading, false);
	...
	checkMasterItem(modulName: string, subMenu?: string, val?: boolean) {
		...
		switch (modulName) {
			case "MasterTable":
				...
				checker = checkarr.find((item: IPrivilegeRole) => item.control.value === true);
	...
	getAllPrivileges(sno: number) {
		...
		let sub = this.systemAdminService.getAllPrivileges(sno).subscribe((res: HttpResponse<IBaseResponse<IUserRolesPrivileges>>) => {
			if (res.body?.status) {
				this.message.toast(res.body!.message!, "success");
	...
	submitPrivilegesForm(form: FormGroup) {
		...
		let sub = this.systemAdminService.editAllPrivileges(dataToSubmit).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
				this.message.toast(res.body!.message!, "success");
				this.eventService.broadcast(reserved.isLoading, false);
			} else this.message.toast(res.body!.message!, "error");
