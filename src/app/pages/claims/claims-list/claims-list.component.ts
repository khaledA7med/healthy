...
import {
  claimsManageCols,
  columnsToExport,
} from "./../../../shared/app/grid/claimsCols";
...
export class ClaimsListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    let sub = this.permissions$.subscribe((res: string[]) => {
      ...
      if (!res.includes(this.uiState.privileges.ChAccessAllUsersClaim))
        this.filterF.savedUser?.patchValue(this.auth.getUser().name!);
    });
  ...
  dataSource: IDatasource = {
    getRows: (params: IGetRowsParams) => {
      ...
      let sub = this.claimService
        .getAllClaims(this.uiState.filters)
        .subscribe((res: HttpResponse<IBaseResponse<IClaims[]>>) => {
  ...
  editPrintedCells(params: ProcessCellForExportParams) {
    if (
      params.column.getColId() === "dateOfLoss" ||
      params.column.getColId() === "dateOfReceive" ||
      params.column.getColId() === "dateOfSubmission" ||
      params.column.getColId() === "dateOfDeadline" ||
      params.column.getColId() === "invoiceDate" ||
      params.column.getColId() === "dateofPayment" ||
  ...
  openFollowUpCanvas(sNo: number) {
    this.offcanvasService.open(this.followUpCanvas, {
      position: "end",
      scroll: false,
    });
  ...
  getFollowUp(sNo: number) {
    let sub = this.claimService
      .getFollowUp(sNo)
      .subscribe((res: HttpResponse<IBaseResponse<IClaimsFollowUp[]>>) => {
  ...
  getClientMail(data: {}) {
    let sub = this.claimService
      .getClientMailData(data)
      .subscribe((res: HttpResponse<IBaseResponse<IEmailResponse>>) => {
