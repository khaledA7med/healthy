...
import { IClaimRejectDeduct, IClaimRejectDeductForm } from "src/app/shared/app/models/Claims/iclaim-reject-deduct-form";
	...
	template: `
		...
		<form [formGroup]="formGroup" (ngSubmit)="onSubmit()">
			<div class="modal-header">
				<h4 class="modal-title" id="modal-basic-title">Rejection/Deduction Details</h4>
				...
				<div class="row">
					<div class="col-lg-6 col-md-12 my-1">
						...
						<ng-select
							class="is-invalid"
							[ngClass]="{
								'is-submitted': (f.type?.touched || submitted) && f.type?.invalid
							}"
							formControlName="type"
							placeholder="Types"
							[items]="types"
						></ng-select>
						<span class="invalid-feedback fw-bold mt-0" *ngIf="(f.type?.touched || submitted) && f.type?.invalid">Required</span>
					</div>
					<div class="col-lg-6 col-md-12 my-1">
						<label for="amount">Amount</label>
						<div class="input-group input-group-sm">
							<input type="text" class="form-control input-text-right" InputMask formControlName="amount" id="amount" placeholder="0.00" />
					...
					<div class="col-lg-6 col-md-12 my-1">
						...
						<input type="text" class="form-control form-control-sm" formControlName="rejectionNote" id="rejectionNote" placeholder="Notes" />
					</div>
				</div>
			</div>
...
export class ClaimRejectDeductFormComponent implements OnInit, OnDestroy {
	...
	@Output() rejectDeductItem: EventEmitter<IClaimRejectDeduct[]> = new EventEmitter<IClaimRejectDeduct[]>();
	...
	validationChecker(): boolean {
		if (this.formEditMode) {
			if (+this.claimAmount - +this.underProcessing + +this.f.amount?.value! > this.claimAmount) {
				this.message.popup("Attention!", "Claim Amount is less than under processing amount", "warning");
				return false;
			}
		}
	...
	onSubmit(): void {
		...
		if (this.formEditMode) {
			...
			formData.append("UnderProcessing", this.underProcessing ? this.underProcessing.toString() : "0");

			this.claimService.saveClaimRejectDeduct(formData).subscribe((res: IBaseResponse<IClaimRejectDeduct[]>) => {
				if (res.status) {
