          ...
          <div class="col-lg-6">
            ...
            <div class="my-1">
              ...
              <input
                ...
                [ngClass]="{
                  'is-invalid':
                    (f.invoiceNo?.touched || submitted) && f.invoiceNo?.invalid
                }"
          ...
          <div class="col-lg-6 col-md-12">
            <div class="my-1">
              ...
              <input
                ...
                [ngClass]="{
                  'is-invalid':
                    (f.WIPNo?.touched || submitted) && f.WIPNo?.invalid
                }"
...
export class ClaimInvoicesFormComponent implements OnInit, OnDestroy {
  ...
  @Output() invoiceList: EventEmitter<IClaimInvoice[]> = new EventEmitter<
    IClaimInvoice[]
  >();
  ...
  totalAmountCalc(): void {
    ...
    this.f.amountDue?.patchValue(
      totals - +this.f.discount?.value! - +this.f.deductibleExcess?.value!
    );
  }
  ...
  sendRequestData(value: FormGroup<IClaimInvoiceForm>) {
    ...
    formData.append(
      "EstimatedCharges",
      val.estimatedCharges ? val.estimatedCharges.toString() : "0"
    );
    formData.append(
      "GrandTotal",
      val.grandTotal ? val.grandTotal.toString() : "0"
    );
    formData.append(
      "DeductibleExcess",
    ...
    formData.append(
      "VATInvoicesAmount",
      val.VATInvoicesAmount ? val.VATInvoicesAmount.toString() : "0"
    );
    formData.append(
      "AmountDue",
      val.amountDue ? val.amountDue.toString() : "0"
    );

    let sub = this.claimService
      .saveClaimInvoice(formData)
      .subscribe((res: IBaseResponse<IClaimInvoice[]>) => {
        if (res.status) {
