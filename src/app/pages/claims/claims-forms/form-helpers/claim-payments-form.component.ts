...
import { IClaimPayment, IClaimPaymentForm } from "src/app/shared/app/models/Claims/iclaim-payment-form";
				...
				<div class="row">
					<div class="col-lg-12 my-1">
						...
						<ng-select
							class="is-invalid"
							[ngClass]="{
								'is-submitted': (f.paymentType.touched || submitted) && f.paymentType.invalid
							}"
							formControlName="paymentType"
							placeholder="Payment Types"
							[items]="paymentTypes"
						></ng-select>
						<span class="invalid-feedback fw-bold mt-0" *ngIf="(f.paymentType.touched || submitted) && f.paymentType.invalid">Required</span>
					</div>
					<div class="col-lg-6 col-sm-12 my-1">
						...
						<div class="input-group input-group-sm">
							<input
								...
								[ngClass]="{
									'is-invalid': (f.amount.touched || submitted) && f.amount.invalid
								}"
								InputMask
					...
					<div class="col-lg-6 col-sm-12 my-1">
						...
						<input type="text" class="form-control form-control-sm" formControlName="IBAN" id="IBAN" placeholder="IBAN" />
					</div>
					<div class="col-lg-6 col-sm-12 my-1">
...
export class ClaimPaymentsFormComponent implements OnInit, OnDestroy {
	...
	@Output() paymentList: EventEmitter<IClaimPayment[]> = new EventEmitter<IClaimPayment[]>();
	constructor(
	...
	initForm(): void {
		this.formGroup = new FormGroup<IClaimPaymentForm>({
			...
			dateofCheque: new FormControl(this.util.dateStructFormat(new Date()) as any),
			dateofPayment: new FormControl(this.util.dateStructFormat(new Date()) as any),
		});
	}
	...
	onSubmit(data: FormGroup<IClaimPaymentForm>): void {
		...
		formData.append("DateofCheque", this.util.dateFormater(val.dateofCheque) as any);
		formData.append("DateofPayment", this.util.dateFormater(val.dateofPayment) as any);
		formData.append("BankName", val.bankName ?? "");

		let sub = this.claimService.saveClaimPayment(formData).subscribe((res: IBaseResponse<IClaimPayment[]>) => {
			if (res.status) {
