...
export class ClaimsFormsComponent implements OnInit, OnDestroy {
  ...
  fillRequestDataToForm(evt: IClaimPolicies): void {
    ...
    this.eventService.broadcast(reserved.isLoading, true);
    let sub = this.claimService
      .searchClientClaimData(evt)
      .subscribe((res: IBaseResponse<IClaimPolicies>) => {
        this.formGroup.patchValue({
  ...
  viewPolicy(): void {
    this.router.navigate([
      {
        outlets: {
          details: [AppRoutes.Production.details, this.uiState.policies.id],
        },
      },
    ]);
  }
