...
export class ClaimsFormsComponent implements OnInit, OnDestroy {
	...
	fillRequestDataToForm(evt: IClaimPolicies): void {
		...
		let sub = this.claimService.searchClientClaimData(evt).subscribe(
			(res: IBaseResponse<IClaimPolicies>) => {
				...
				this.f.dateOfLossFrom?.patchValue(this.util.dateStructFormat(evt.periodFrom) as any);
				this.f.dateOfLossTo?.patchValue(this.util.dateStructFormat(evt.periodTo) as any);
	...
	openViewReport(data?: IClaimPayment) {
		...
		let sub = this.claimService.viewArchiveNoteReport(newData).subscribe(
