...
export class PoliciesFormsComponent implements OnInit, OnDestroy {
	...
	fillRequestDataToForm(e: IPolicyRequests) {
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.productionService.fillRequestData(e.policySerial!, e.clientPolicySNo!).subscribe(
	...
	fillPolicyDataToForm(e: IPoliciesRef) {
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.productionService.loadPolicyData(e.policiesSNo, e.sNo).subscribe(
	...
	setPolicyDataToForm(data: IPolicyPreview): void {
		...
		this.f.issueDate?.patchValue(this.appUtils.dateStructFormat(data?.issueDate!) as any);
		this.f.periodFrom?.patchValue(this.appUtils.dateStructFormat(data?.periodFrom!) as any);
		this.f.periodTo?.patchValue(this.appUtils.dateStructFormat(data?.periodTo!) as any);
		...
		let sub = this.productionService.savePolicy(formData).subscribe(
			(res: HttpResponse<IBaseResponse<number>>) => {
				if (res.body?.status) {
					...
					if (this.uiState.editId) this.router.navigate([AppRoutes.Production.base]);
					this.resetForm();
