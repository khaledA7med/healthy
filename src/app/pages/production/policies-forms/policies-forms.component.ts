    ...
    let sub = this.productionService
      ...
      .subscribe((res: IBaseResponse<IPolicyRequestResponse>) => {
        if (res.status) {
          ...
          this.f.issueType?.patchValue(
            this.uiState.policy.issueType.endorsement
          );
    ...
    this.productionService
      ...
      .subscribe((res: HttpResponse<IBaseResponse<IPolicyPreview>>) => {
        if (res.body?.status) {
          ...
          if (data.issueType === this.uiState.policy.issueType.endorsement)
            this.endorsementIssue();
          this.formGroup.patchValue({
            sNo: data.sNo,
  ...
  sendFormRequestData(): void {
    ...
    for (let i = 0; i < terms.length; i++) {
      ...
      formData.append(
        ...
        terms[i].percentage?.toString()! ?? "0"
      );
      formData.append(
        `PaymentTermsList[${i}].amount`,
        terms[i].amount?.toString()! ?? "0"
      );
      formData.append(
        `PaymentTermsList[${i}].policyFees`,
        terms[i].policyFees?.toString()! ?? "0"
      );
      formData.append(
        `PaymentTermsList[${i}].vatAmount`,
        terms[i].vatAmount?.toString()! ?? "0"
      );
    }
    ...
    for (let i = 0; i < commissions.length; i++) {
      ...
      formData.append(
        ...
        commissions[i].percentage?.toString()! ?? "0"
      );
      formData.append(
        `ProducersCommissionsList[${i}].amount`,
        commissions[i].amount?.toString()! ?? "0"
      );
    }
