...
import { IPolicyCommissionListForms, IPolicyPaymentsListForms } from "src/app/shared/app/models/Production/ipolicy-payments";
...
export class PoliciesFormsComponent implements OnInit, OnDestroy {
		...
		let sub = this.route.paramMap.subscribe((res) => {
			if (res.get("id")) {
				...
				let invoice = atob(res.get("invoice")!),
					...
					endorsType = atob(res.get("endorsType")!),
	...
	formDataHandler(): void {
		...
		let sub = this.formData.subscribe((res) => {
			this.uiState.producerCommission.producers = res.Producers?.content.filter((el) => !el.name.startsWith("Direct Business"));
		});
	...
	initForm(): void {
		this.formGroup = new FormGroup<IProductionForms>({
			...
			policyHolder: new FormControl({ value: null, disabled: true }, Validators.required),
			...
			endorsType: new FormControl({ value: null, disabled: true }, Validators.required),
			endorsNo: new FormControl({ value: null, disabled: true }, Validators.required),
			insurComp: new FormControl(null, Validators.required),
			className: new FormControl(null, Validators.required),
			lineOfBusiness: new FormControl(null, Validators.required),
			minDriverAge: new FormControl({ value: null, disabled: true }, Validators.required),
			...
			numberOfBeneficiaries: new FormControl(null, [Validators.required, Validators.min(1)]),
			...
			producersCommissionsList: new FormArray<FormGroup<IPolicyCommissionListForms>>([]),
	...
	searchByEvt(): void {
		if (this.f.searchType?.value === this.uiState.policy.searching.request) this.f.requestNo?.setValidators(Validators.required);
		else {
			this.resetForm();
			this.f.searchType?.patchValue("Client");
			this.f.issueType?.patchValue(this.uiState.policy.issueType.new);
	...
	issueDate(e: any) {
		...
		if (this.f.periodTo?.enabled) this.f.periodTo?.patchValue(this.expiryDateCalc(e).gon);
	}
	...
	fillRequestDataToForm(e: IPolicyRequests) {
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.productionService.fillRequestData(e.policySerial!, e.clientPolicySNo!).subscribe((res: IBaseResponse<IPolicyRequestResponse>) => {
			if (res.status) {
				...
				this.f.issueType?.patchValue(this.uiState.policy.issueType.endorsement);
	...
	fillPolicyDataToForm(e: IPoliciesRef) {
		this.eventService.broadcast(reserved.isLoading, true);
		let sub = this.productionService.loadPolicyData(e.policiesSNo, e.sNo).subscribe((res: IBaseResponse<IPolicyPreview>) => {
			if (res.status) {
				let data = res.data;
				this.setPolicyDataToForm(data!);
				this.uiState.policy.isRequest = false;
	...
	getPolicy(id: string): void {
		this.uiState.editMode = true;
		this.productionService.getPolicy(id).subscribe((res: HttpResponse<IBaseResponse<IPolicyPreview>>) => {
			if (res.body?.status) {
				let data = res.body?.data!;
				if (data.issueType === this.uiState.policy.issueType.endorsement) this.endorsementIssue();
				this.formGroup.patchValue({
					sNo: data.sNo,
	...
	setPolicyDataToForm(data: IPolicyPreview): void {
		this.formGroup.patchValue({
			...
			endorsType: data.endorsType ?? null,
		...
		if (data?.producersCommissionsList!) data?.producersCommissionsList!.forEach((com) => this.addProducerCommission(com));
		if (data?.paymentTermsList!) data?.paymentTermsList!.forEach((pay) => this.addPaymentTerm(pay));
	...
	vatHandler(): void {
		if (this.f.endorsType?.value === "Refund" && !this.f.deductFees?.value) {
			this.f.vatValue?.patchValue((+this.f.netPremium?.value! - +this.f.fees?.value!) * (+this.f.vatPerc?.value! / 100));
			this.f.totalPremium?.patchValue(+this.f.netPremium?.value! - +this.f.fees?.value! + +this.f.vatValue?.value!);
		} else {
			this.f.vatValue?.patchValue(
				this.appUtils.currencyFormater((+this.f.netPremium?.value! + +this.f.fees?.value!) * (this.f.vatPerc?.value! / 100))
			);
			this.f.totalPremium?.patchValue(+this.f.netPremium?.value! + +this.f.fees?.value! + +this.f.vatValue?.value!);
		}

		if (+this.f.compCommPerc?.value! > 0)
			this.f.compCommVAT?.patchValue(this.appUtils.currencyFormater(+this.f.compCommAmount?.value! * (this.f.vatPerc?.value! / 100)));
	}
	...
	calcHandlers(): void {
		...
		let sub2 = this.f.compCommPerc?.valueChanges.subscribe((el) => this.commissionsValues());
		let sub3 = this.f.producerCommPerc?.valueChanges.subscribe((el) => this.commissionsValues());
		this.subscribes.push(sub!);
	...
	commissionsValues(): void {
		this.f.compCommAmount?.patchValue(this.appUtils.currencyFormater(+this.f.netPremium?.value! * (+this.f.compCommPerc?.value! / 100)));
		this.f.compCommVAT?.patchValue(this.appUtils.currencyFormater(+this.f.compCommAmount?.value! * (this.f.vatPerc?.value! / 100)));
		this.f.producerComm?.patchValue(this.appUtils.currencyFormater(this.f.compCommAmount?.value! * (this.f.producerCommPerc?.value! / 100)));
	...
	addPaymentTerm(data?: IPaymentTermsList) {
		...
		let payment = new FormGroup<IPolicyPaymentsListForms>({
			payDate: new FormControl((this.appUtils.dateStructFormat(data?.payDate!) as any) || null, Validators.required),
			amount: new FormControl(data?.amount || null),
			percentage: new FormControl(data?.percentage || null, [Validators.max(100), Validators.min(0)]),
	...
	totalPaymentRow() {
		...
		this.f.paymentTermsList?.controls.forEach((el) => {
			let sub1 = el.controls.percentage?.valueChanges.subscribe((elm) => {
				if (+elm! > this.uiState.paymentTermHandler.totalPerc) el.controls.percentage?.patchValue(this.uiState.paymentTermHandler.totalPerc, handler);
				el.controls.amount?.patchValue(this.appUtils.currencyFormater(+this.f.netPremium?.value! * (+el.controls.percentage?.value! / 100)));
				el.controls.policyFees?.patchValue(this.appUtils.currencyFormater(+this.f.fees?.value! * (+el.controls.percentage?.value! / 100)));
				el.controls.vatAmount?.patchValue(this.appUtils.currencyFormater(+this.f.vatValue?.value! * (+el.controls.percentage?.value! / 100)));

				if (+el.controls.policyFees?.value! >= +this.uiState.paymentTermHandler.fees) {
					el.controls.policyFees?.patchValue(+this.uiState.paymentTermHandler.fees);
				}

				if (+el.controls.vatAmount?.value! >= +this.uiState.paymentTermHandler.vat) {
					el.controls.vatAmount?.patchValue(+this.uiState.paymentTermHandler.vat);
				}
		...
		let sub = this.paymentTermsArrayControls.valueChanges.subscribe((el) => {
			for (let i = 0; i < el.length; i++) {
				this.paymentsControls(i, "rowTotal")?.patchValue(+el[i].amount + +el[i].vatAmount + +el[i].policyFees, handler);
			}
			this.uiState.paymentTermsTotals = {
				percentage: el.reduce((prev: any, next: any) => +prev + +next.percentage, 0),
				netPremium: el.reduce((prev: any, next: any) => +prev + +next.amount, 0),
				fees: el.reduce((prev: any, next: any) => +prev + +next.policyFees, 0),
				vat: el.reduce((prev: any, next: any) => +prev + +next.vatAmount, 0),
				total: el.reduce((prev: any, next: any) => +prev + +next.amount + +next.policyFees + +next.vatAmount, 0),
			};
		});
		this.subscribes.push(sub);
	}
	...
	paymentTermsHandler(): void {
		...
		if (len) {
			for (let i = 0; i < len; i++) {
				...
				net.patchValue(this.appUtils.currencyFormater(this.f.netPremium?.value! * (perc / 100)));
			}
		}
	}
	...
	paymentTermsValueHandler(i: number, con: string, e: Event) {
		...
		if (+elm.replace(/,/g, "") > +val) {
			this.paymentsControls(i, con).patchValue(this.appUtils.currencyFormater(val));
			return;
		}
	}

	paymentRecalcEvtHandler(): void {
		this.uiState.paymentTermHandler.totalPerc = 100 - +this.uiState.paymentTermsTotals.percentage;
		this.uiState.paymentTermHandler.fees = +this.f.fees?.value! - +this.uiState.paymentTermsTotals.fees;
		this.uiState.paymentTermHandler.vat = +this.f.vatValue?.value! - +this.uiState.paymentTermsTotals.vat;
	}

	equalizerPaymentTerms(): void {
		let cls = this.paymentTermsArrayControls,
			net = +this.f.netPremium?.value! - this.uiState.paymentTermsTotals.netPremium,
			fees = +this.f.fees?.value! - this.uiState.paymentTermsTotals.fees,
			vat = +this.f.vatValue?.value! - this.uiState.paymentTermsTotals.vat,
			total = +this.f.totalPremium?.value! - this.uiState.paymentTermsTotals.total;
	...
	get paymentTermsListBool(): boolean {
		return (
			...
			(+this.f.totalPremium?.value! > 0 && this.uiState.paymentTermsTotals.percentage === 100)
		);
	}

	removePayment(i: number) {
		this.uiState.paymentTermHandler.totalPerc += this.paymentsControls(i, "percentage").value;
		this.uiState.paymentTermHandler.fees += this.paymentsControls(i, "policyFees").value;
		this.uiState.paymentTermHandler.vat += this.paymentsControls(i, "vatAmount").value;
	...
	addProducerCommission(data?: IProducersCommissionsList): void {
		...
		let commission = new FormGroup<IPolicyCommissionListForms>({
			...
			amount: new FormControl(data?.amount || null),
			percentage: new FormControl(data?.percentage || null, [Validators.max(100), Validators.min(0), Validators.required]),
			rowTotal: new FormControl(null),
		});
		...
		if (this.f.producersCommissionsList?.length === 0 && !this.f.producer?.value?.startsWith("Direct Business"))
	...
	totalCommissionRow() {
		...
		this.f.producersCommissionsList?.controls.forEach((el) => {
			let sub1 = el.controls.percentage?.valueChanges.subscribe((elm) => {
				if (+elm! > +this.f.producerCommPerc?.value!) el.controls.percentage?.patchValue(+this.f.producerCommPerc?.value!, handler);
				el.controls.amount?.patchValue(this.appUtils.currencyFormater(+this.f.compCommAmount?.value! * (+el.controls.percentage?.value! / 100)));
			});
			this.subscribes.push(sub1!);
		});
		let sub = this.producersCommissionsArrayControls.valueChanges.subscribe((el) => {
	...
	commissionHandler(): void {
		if (this.f.producersCommissionsList?.length) {
			for (let i = 0; i < this.producersCommissionsArrayControls.length; i++) {
				...
				net.patchValue(this.appUtils.currencyFormater(this.f.producerComm?.value! * (perc / 100)));
			}
		}
	}
	...
	get commissionListBool(): boolean {
		return (
			...
			+this.uiState.producerCommission.commissionTotals.percentage >= +this.f.producerCommPerc?.value!
		);
	}

	removeCommission(i: number) {
		this.uiState.producerCommission.commissionTotals.percentage += this.commissionControls(i, "percentage").value;
		this.uiState.producerCommission.commissionTotals.amount += this.commissionControls(i, "amount").value;
	...
	paymentTermChecker(): boolean {
		...
		if (totals.fees !== values.fees || totals.netPremium !== values.net || totals.vat !== values.vat) {
			if (this.f.endorsType?.value !== "Refund" && this.f.endorsType?.value !== "Cancellation") {
				this.message.popup("Attention!", "Payment Schedule Totals Not Correct!", "warning");
				return false;
			}
			return true;
		}
	...
	producerCommissionChecker(): boolean {
		...
		if (coms.amount !== values.producerCom || coms.percentage !== values.producerComPerc) {
			this.message.popup("Attention!", "Producer Commissions Schedule Totals Not Correct!", "warning");
			return false;
		}
		return true;
	}
	...
	financeValueChecker(): void {
		...
		if (+this.f.compCommAmount?.value! === 0) arr.push("Company Commission Value = 0");

		if (arr.length) {
			for (let i = 0; i < arr.length; i++) msg += `<li class='text-start'>${arr[i]}</li>`;

			this.message
				.templateComfirmation("Are You Sure Want Countinue ?!", `<ul>${msg}</ul>`, "Yes, Sure !", undefined, "question")
		...
		if (this.f.issueType?.value === issueType.endorsement) {
			let sub = this.productionService
				...
				.subscribe((res: HttpResponse<IBaseResponse<number>>) => {
					if (res.body?.status) {
						this.eventService.broadcast(reserved.isLoading, false);
						this.message.templateComfirmation("Attention!", res.body.message!, "Continue", "info", "question").then((res: SweetAlertResult) => {
	...
	sendFormRequestData(): void {
		...
		formData.append("NumberOfBeneficiaries", val.numberOfBeneficiaries?.toString() ?? "0");
		...
		formData.append("ProducerCommPerc", val.producerCommPerc?.toString()! ?? "");
		...
		for (let i = 0; i < terms.length; i++) {
			formData.append(`PaymentTermsList[${i}].payDate`, this.appUtils.dateFormater(terms[i].payDate));
			formData.append(`PaymentTermsList[${i}].percentage`, terms[i].percentage?.toString()! ?? "0");
			formData.append(`PaymentTermsList[${i}].amount`, terms[i].amount?.toString()! ?? "0");
			formData.append(`PaymentTermsList[${i}].policyFees`, terms[i].policyFees?.toString()! ?? "0");
			formData.append(`PaymentTermsList[${i}].vatAmount`, terms[i].vatAmount?.toString()! ?? "0");
		}

		let commissions = val.producersCommissionsList!;
		for (let i = 0; i < commissions.length; i++) {
			formData.append(`ProducersCommissionsList[${i}].producer`, commissions[i].producer!);
			formData.append(`ProducersCommissionsList[${i}].percentage`, commissions[i].percentage?.toString()! ?? "0");
			formData.append(`ProducersCommissionsList[${i}].amount`, commissions[i].amount?.toString()! ?? "0");
		}

		this.documentsToUpload.forEach((el) => formData.append("Documents", el));

		let sub = this.productionService.savePolicy(formData).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
				this.message.toast(res.body.message!, "success");
				if (this.uiState.editMode) this.router.navigate([AppRoutes.Production.base]);
				this.resetForm();
