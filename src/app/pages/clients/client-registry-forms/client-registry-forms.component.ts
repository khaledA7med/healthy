...
export class ClientRegistryFormsComponent implements OnInit, OnDestroy {
  ...
  initForm(): void {
    this.formGroup = new FormGroup<IClientForms>({
      ...
      screeningResult: new FormControl(""),
  ...
  addContact(data?: IClientContact): void {
    ...
    let contact = new FormGroup<IClientContact>({
      ...
      mobile: new FormControl(data?.mobile || null),
      tele: new FormControl(data?.tele || null),
  ...
  getClient(id: string): void {
    ...
    let sub = this.clientService
      ...
      .subscribe((res: HttpResponse<IBaseResponse<IClientPreview>>) => {
        if (res.body?.status) this.patchEditingClient(res.body?.data!);
        else {
