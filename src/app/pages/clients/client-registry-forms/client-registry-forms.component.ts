...
import { IClientCorporateForm, IClientForms, IClientRetailForm } from "src/app/shared/app/models/Clients/iclientForms";
...
export class ClientRegistryFormsComponent implements OnInit, OnDestroy {
	...
	initForm(): void {
		this.formGroup = new FormGroup<IClientForms>({
			...
			corporate: new FormGroup<IClientCorporateForm>({
				...
				sponsorID: new FormControl(null, [Validators.minLength(20), Validators.maxLength(20)]),
				unifiedNo: new FormControl(""),
				vatNo: new FormControl(null, [Validators.minLength(15), Validators.maxLength(15), Validators.required]),
			...
			// Contact Details
			tele: new FormControl("", [Validators.required, Validators.pattern("[0-9]{9}")]),
	...
	addContact(data?: IClientContact): void {
		...
		let contact = new FormGroup<IClientContact>({
			contactName: new FormControl(data?.contactName || null, Validators.required),
			position: new FormControl(data?.position || null),
			extension: new FormControl(data?.extension || null),
			mobile: new FormControl(data?.mobile || null, Validators.pattern("[0-9]{9}")),
	...
	getClient(id: string): void {
		...
		let sub = this.clientService.getClientById(id).subscribe((res: HttpResponse<IBaseResponse<IClientPreview>>) => {
			if (res.body?.status) {
				this.patchEditingClient(res.body?.data!);
			} else {
				this.eventService.broadcast(reserved.isLoading, false);
				this.message.popup("Error", res.body?.message!, "error");
			}
		});
		this.subscribes.push(sub);
	}
	...
	patchEditingClient(client: IClientPreview): void {
		...
		this.formGroup.patchValue({
			...
			corporate: {
				...
				dateOfIncorporation: this.util.dateStructFormat(client.dateOfIncorporation!) as any,
				dateOfIncorporationHijri: this.util.dateStructFormat(client.dateOfIncorporationHijri!) as any,

				// Expiry Date
				expiryDate: this.util.dateStructFormat(client.expiryDate!) as any,
				expiryDateHijri: this.util.dateStructFormat(client.expiryDateHijri!) as any,
			},
	...
	onSubmit(clientForm: FormGroup<IClientForms>) {
		...
		formData.append("IDExpiryDate", this.util.dateFormater(val.retail?.idExpiryDate ?? ""));
		formData.append("Nationality", val.retail?.nationality ?? "");
		formData.append("SourceofIncome", val.retail?.sourceofIncome ?? "");

		// Corporate
		formData.append("RegistrationStatus", val.corporate?.registrationStatus ?? "");
		formData.append("BusinessActivity", val.corporate?.businessActivity ?? "");
		formData.append("MarketSegment", val.corporate?.marketSegment ?? "");
		formData.append("DateOfIncorporation", this.util.dateFormater(val.corporate?.dateOfIncorporation ?? ""));
		formData.append("DateOfIncorporationHijri", this.util.dateFormater(val.corporate?.dateOfIncorporationHijri ?? ""));
		formData.append("CommericalNo", val.corporate?.commericalNo ?? "");
		formData.append("ExpiryDate", this.util.dateFormater(val.corporate?.expiryDate ?? ""));
		formData.append("ExpiryDateHijri", this.util.dateFormater(val.corporate?.expiryDateHijri ?? ""));
		...
		for (let i = 0; i < contacts.length; i++) {
			formData.append(`ClientContacts[${i}].contactName`, contacts[i].contactName!);
			formData.append(`ClientContacts[${i}].mobile`, contacts[i].mobile ?? "");
			formData.append(`ClientContacts[${i}].lineOfBusiness`, contacts[i].lineOfBusiness ?? "");
			formData.append(`ClientContacts[${i}].department`, contacts[i].department ?? "");
		...
		for (let i = 0; i < banks.length; i++) {
			...
			formData.append(`ClientsBankAccounts[${i}].arabicName`, banks[i].arabicName ?? "");
			...
			formData.append(`ClientsBankAccounts[${i}].swiftCode`, banks[i].swiftCode ?? "");
		}
		...
		let sub = this.clientService.saveClient(formData).subscribe((res: HttpResponse<IBaseResponse<number>>) => {
			if (res.body?.status) {
